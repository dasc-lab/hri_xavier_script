...
bool turn_target_reached(float yaw_error) {
	...
	return fabs(yaw_error) < 0.05;
}

Vector2f RoverControl::turn_controller(float  yaw_error) {
	Vector2f lin_ang_cmd;

	float angular_vel_cmd = komega * yaw_error;
	lin_ang_cmd(0) = 0;
...
Vector2f RoverControl::rover_controller_default(){
	...
	float yaw_error = wrap_angle(yaw_ref - rover_yaw);
	if (!turn_target_reached(yaw_error)) {
		// turn
	} else {
		// straight
	}
